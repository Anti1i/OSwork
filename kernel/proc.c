...
void retaddress_error() {
    disp_str("\n\n\n\n\n\n\n\n\n\n\n");
    disp_str("here checked a return address Error: ");
    disp_str(p_proc_ready->name);
    disp_str("  is hostile\n");
}

PUBLIC int sys_check_stack(int _unused,
                           int _unused2,
                           char* _unused3,
                           struct proc* p_proc) {
    // disp_str(p_proc->name);
    ...
    // disp_str("\n");
// 返回地址超出进程镜像空间（通常说明栈溢出或被破坏）
    if (retaddr > (PROC_IMAGE_SIZE_DEFAULT - PROC_ORIGIN_STACK)) {
        // 如果在栈上  或者超过栈了 ,或者
        retaddress_error();
    }
   //if (retaddr - 0x1000 > 0x1500 || retaddr < 0x1000) {  // 超长距离跳转
       // retaddress_error();
    if (retaddr < 0x1000) { 
        retaddress_error();
    }
...
PUBLIC void dump_proc(struct proc* p) {
    ...
    disp_color_str(info, text_color);
    /* sprintf(info, "nr_tty: 0x%x.  ", p->nr_tty); disp_color_str(info,
     * text_color); */
    disp_color_str("\n", text_color);
...
PUBLIC void dump_msg(const char* title, MESSAGE* m) {
    int packed = 0;
    printl(
        "{%s}<0x%x>{%ssrc:%s(%d),%stype:%d,%s(0x%x, 0x%x, 0x%x, 0x%x, 0x%x, "
        "0x%x)%s}%s",  //, (0x%x, 0x%x, 0x%x)}",
        title, (int)m, packed ? "" : "\n        ", proc_table[m->source].name,
        m->source, packed ? " " : "\n        ", m->type,
        packed ? " " : "\n        ", m->u.m3.m3i1, m->u.m3.m3i2, m->u.m3.m3i3,
        m->u.m3.m3i4, (int)m->u.m3.m3p1, (int)m->u.m3.m3p2, packed ? "" : "\n",
        packed ? "" : "\n" /* , */
    );
}
