...
PUBLIC void clock_handler(int irq) {
    ...
    if ((p_proc_ready - &FIRST_PROC >= 0xb) && DYNAMIC_CHECK)  // 仅对用户进程且排除INIT
    {
        check_stack();  // 在调度之前实现 , 查看当前进程栈
        SYSlog = 1;
    }
    ...
    schedule();
    //if (p_proc_ready - &FIRST_PROC >= 0xb && DYNAMIC_CHECK)  // 仅对用户进程
    //{
     //   check_stack();  // 在调度之hou实现 , 查看当前进程栈
    //}
