    ...
    while (1) {
        ...
        switch (msgtype) {
            ...
            case STAT:
                ...
                break;
            case SEARCH:
                // printl("fs_msg.pBug in main address is %d\n", fs_msg.pBUF);
                // printl("BUF in main: %s\n", fs_msg.pBUF);
                do_search_dir();
                break;
            case PROPRINT:
                // printl("fs_msg.pBug in main address is %d\n", fs_msg.pBUF);
                // printl("BUF in main: %s\n", fs_msg.pBUF);
                //【WSH】
                dump_fd_graph(0);// 传入NULL，表示将进程表内容写入fs_msg.pBUF
            ...
            case PRO_KILL:
                ...
                //【WSH】
		printl("{%s}\n", fs_msg.pBUF);
                if(strlen(fs_msg.pBUF) == 1){
                    ret = (fs_msg.pBUF[0] - '0');// 传入单字符pid
                }
                else if(strlen(fs_msg.pBUF) == 2){
                    ret =(fs_msg.pBUF[0] - '0') *10 + (fs_msg.pBUF[1] - '0');// 传入两字符pid
                }
                else{
                    // 超过两位数的pid暂不支持
		...
		printl("{%d}\n", ret);
                dump_fd_graph(ret);// 传入fs_msg.pBUF,企图杀死进程
                // if(ret == 1){
                //     memset(fs_msg.pBUF, 0, sizeof(fs_msg.pBUF));
                ...
                // {
                //     memset(fs_msg.pBUF, 0, sizeof(fs_msg.pBUF));
                //     sprintf(fs_msg.pBUF, "Cannot kill system process or native process!:%s\n", fs_msg.pBUF);
                // }
                // else{
                //     memset(fs_msg.pBUF, 0, sizeof(fs_msg.pBUF));
	    ...
	    if(KEYlog !=0){syslog("---TTY--: switched to TTY%d\n", KEYlog-1);}
        KEYlog = 0;
	#endif
#ifdef ENABLE_DISK_LOG
	    if(SYSlog !=0){syslog("---SYS--: check\n");}
        SYSlog = 0;
#endif  
...
//         switch (msgtype) {
//             case UNLINK:
//                 dump_fd_graph("%s just finished. (pid:%d)", msg_name[msgtype],
//                               src);
//                 // panic("");
//             case OPEN:
//             case CLOSE:
...
PRIVATE void mkfs() {
    ...
    printl(
        ...
        geo.base * 2, (geo.base + 1) * 2, (geo.base + 1 + 1) * 2,
    ...
    int bit_offset = INSTALL_START_SECT - sb.n_1st_sect +
 ...
 *****************************************************************************/
PUBLIC int rw_sector(int io_type,
                     int dev,
                     u64 pos,
                     int bytes,
                     int proc_nr,
...
PRIVATE int fs_fork() {
    ...
    #ifdef ENABLE_DISK_LOG
	syslog("PROCESS: forked new process, child pid: %d\n", fs_msg.PID);
	#endif
    return 0;
}
...
PRIVATE int fs_exit() {
    #ifdef ENABLE_DISK_LOG
	syslog("PROCESS: exit process, pid: %d\n", fs_msg.PID);	
	#endif
    int i;
