 ...
 *****************************************************************************/
//const char *str
PUBLIC int dump_fd_graph(int str)
{
	if(str == 0) {
	// str为NULL时，将进程表内容写入fs_msg.pBUF
	char* buf = fs_msg.pBUF; // 目标缓冲区
	int len = 0;
	int i;
	struct proc* p_proc = proc_table;
	// 状态字符串映射
	const char* state_str[8] = {
		"RUNNING", "SENDING", "RECEIVING", "WAITING",
		"HANGING", "FREE_SLOT", "UNKNOWN", "UNKNOWN"
	};
	// 表头
	len += sprintf(buf + len, "PID-Name-State-Parent-Prio-Ticks\n");
	for (i = 0; i < NR_TASKS + NR_PROCS; i++, p_proc++) {
		if (p_proc->p_flags == FREE_SLOT) continue; // 跳过空槽
		int flag = p_proc->p_flags;
		const char* state = "RUNNING";
		if (flag == 0) state = "RUNNING";
		else if (flag == 1) state = "SENDING";
		else if (flag == 2) state = "RECEIVING";
		else if (flag == 3) state = "WAITING";
		else if (flag == 4) state = "HANGING";
		else if (flag == 5) state = "FREE_SLOT";
		else state = "UNKNOWN";
		len += sprintf(buf + len, "%d    %s    %s   %d    %d    %d\n",
			i, p_proc->name, state, p_proc->p_parent, p_proc->priority, p_proc->ticks);
		if (len > STR_DEFAULT_LEN - 64) break; // 防止缓冲区溢出
	}
	buf[STR_DEFAULT_LEN-1] = '\0'; // 保证字符串结尾
		return 0;
	}
	else
	{
		...
		// 	sprintf(fs_msg.pBUF, "Invalid PID format!\n");
		// 	return 1; // 非法pid
		// }
		assert(pid < 100); // 简单保护

		// 只允许杀用户进程
		if (pid <= NR_TASKS || pid == 9 || pid == 10) {
			memset(fs_msg.pBUF, 0, sizeof(fs_msg.pBUF));
			sprintf(fs_msg.pBUF, "Cannot kill system process or native process!:%d\n", pid);
			return 2; // 系统任务或本地进程，不允许删除
		}
		if (pid >= NR_TASKS + NR_PROCS) {
		    memset(fs_msg.pBUF, 0, sizeof(fs_msg.pBUF));
		    sprintf(fs_msg.pBUF, "No such process!\n");
			return 1; // 非法pid
		}
		// 在进程表中查找 pid 对应的进程
		...
		for (i = 0; i < NR_FILES; i++) {
			if (p_proc->filp[i]) {
				struct file_desc* fd = p_proc->filp[i];
				p_proc->filp[i] = 0;
				if (fd->fd_cnt > 0) {
					fd->fd_cnt--;
					if (fd->fd_cnt == 0) {
		...
		// 3. 其他资源可根据需要补充释放
		    memset(fs_msg.pBUF, 0, sizeof(fs_msg.pBUF));
	    sprintf(fs_msg.pBUF, "Process killed successfully!\n");
		return 0;
	}
	...
	// 	struct proc* p_proc = proc_table;
	// 	// 写入表头
	// 	len += sprintf(buf + len, "PID   Name        Flags\n");
	// 	// 遍历进程表，将每个进程的信息写入缓冲区
	// 	for (i = 0; i < NR_TASKS + NR_PROCS; i++, p_proc++) {
	// 		if (p_proc->p_flags == FREE_SLOT) continue; // 跳过空槽
	// 		len += sprintf(buf + len, "%3d   %-10s  0x%02x\n", i, p_proc->name, p_proc->p_flags); // 写入PID、进程名、标志
	// 		if (len > STR_DEFAULT_LEN - 32) break; // 防止缓冲区溢出
	// 	}
	// 	buf[STR_DEFAULT_LEN-1] = '\0'; // 保证字符串结尾
	// 	return;
	// }	
